...
2. cs: cross-section operator: will require "univ", "group_code" (if necessary)
...
def ts_avg_of_nmax(data:np.array, formation_period:int, sample:int) -> np.array:

    max_arr = np.full_like(data, np.nan)

    for i in range(formation_period, data.shape[0]):
        
        subset = data[i-formation_period:i, :]
        subset = np.sort(subset, axis=0)

        max_arr[i,:] = np.mean(subset[-sample:,:], axis=0)

    valid = pd.DataFrame(data).rolling(formation_period, min_periods=int(formation_period*0.8)).apply(lambda x:1, raw=True)
    valid = valid==1
    max_arr[~valid] = np.nan

    return max_arr

@njit
def ts_ars(input, length, unit):
	allt, alls = input.shape
	res = np.full((allt, alls), np.nan)
	valid = np.isfinite(input)
	unit_counts = length//unit
	for s in prange(alls):
		for t in range(length-1, allt):
			if valid[t, s]:
				xt = input[t-length+1: t+1, s]
				rs_sum = 0
				for flag in range(0, length, unit):
					xt_uk = xt[flag:flag+unit]
					yt_uk = xt_uk - np.mean(xt_uk)
					cum_yt_uk = np.cumsum(yt_uk)
					r_uk = np.max(cum_yt_uk) - np.min(cum_yt_uk)
					s_uk = np.std(yt_uk)
					rs_uk = r_uk/s_uk
					if np.isfinite(rs_uk)==False:
						rs_uk = 0
					rs_sum += rs_uk
				ars_u = rs_sum / unit_counts
				res[t, s] = ars_u
	return res
